...
function(req, anns = "" ) {
  ...
  # JsonData <- fromJSON(file= url )
  # 
  # write_rds(anns, paste0(baseFolder,"anns.rds"))
  # 
  # anns <- read_rds(paste0(baseFolder,"anns.rds"))
  
  annotations <- anns$annotation %>% as.data.frame() %>% mutate(docid = anns$docid,page = anns$page,user = anns$user,corrupted = anns$corrupted,tableType = anns$tableType) %>% select(user,docid,page,corrupted,tableType,location,number,content,qualifiers) %>%
