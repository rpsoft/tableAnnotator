...
tablesDirectory <- "~/ihw/tableAnnotator/backupdata/annotations/Single_table_sheets/"

################# PREPARING THE INPUT VARIABLE annotations.
#
new_obj <- readRDS("~/ihw/tableAnnotator/tools/RDS_TO_HTML/newTables/clean_full_tables_rds_jul_2019.rds") ## This one holds styles for tables for which we do not have the xlsx
...
annotations <- formatAnnotations(read_csv("~/ihw/tableAnnotator/backupdata/annotations/all_annotations_jul_2019.csv") %>% mutate(corrupted = if_else(str_length(corrupted_text) < 1 | is.na(corrupted_text), FALSE, TRUE ) ) )


runAll <- function(annotations,new_obj){
...
# 
ann <- annotations %>% filter(docid == "9892586" & page == 3 & user == "Hebe")

ann <- annotation_bug %>% mutate(corrupted = TRUE) %>% select(annotations %>% colnames )
...
write_rds(l, "~/ihw/tableAnnotator/backupdata/annotations/result.rds")
write_rds(err, "~/ihw/tableAnnotator/backupdata/annotations/errors.rds")
...
docsIndex <- docsIndex[order(docsIndex$docid, docsIndex$page, docsIndex$user),]
...
# dir_files_df %>% left_join(all_good_annotations) %>% View
