...
html_2_df <- function (pmid,page){
  ...
  if(str_length(textHeader) > 0){
    ...
    header <- header %>% rbind(data_frame(path=c("header"), character=c(NA), attr= c(NA), row=c(2), col=c(1:maxcol)))
...
runAll <- function(annotations){
  ...
  TidyTable <- function(docid_page_selected){
    ...
    if(any(meta$location == "Col" & meta$number ==2)){
        print("Note two columns here, this is unusual")
    }
...
annotations1 <- read_csv("~/ihw/tableAnnotator/annotations/first/first_batch_results.csv") %>% mutate( docid = str_extract(docid, "^([0-9])+") )
annotations2 <- read_csv("~/ihw/tableAnnotator/annotations/second/second_annotations.csv") %>% mutate( docid = str_extract(docid, "^([0-9])+") )
annotations3 <- read_csv("~/ihw/tableAnnotator/annotations/third/third_batch_results.csv") %>% mutate( docid = str_extract(docid, "^([0-9])+") )

results1 <- runAll(annotations1)
results2 <- runAll(annotations2)
results3 <- runAll(annotations3)

all_ids <- annotations %>% mutate(docid_page = paste0(docid,"_",page)) %>% distinct(docid_page) 
annotated_ids <- results %>% distinct(docid_page)

all_ids %>% nrow
annotated_ids %>% nrow

failed_annotated <- all_ids %>% anti_join(annotated_ids)

failed_tables <- annotations %>% mutate(docid_page = paste0(docid,"_",page)) %>% filter ( docid_page %in% failed_annotated$docid_page)
failed_tables  %>% distinct(docid_page) %>% nrow ## 96 tables failed to extract from annotations

failed_tables <- failed_tables %>% mutate ( number = ifelse(docid_page == "19125778_2" & location == "Col", 1, number )) %>% filter ( docid_page == "19125778_2")

failed_tables %>% distinct(docid_page)

results <- runAll(failed_tables)


# results %>% as_tibble() %>% select(-col,-row,-value) %>% write.csv("first/extracted.csv",row.names=FALSE)
